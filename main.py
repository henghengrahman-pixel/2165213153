...
FONT_PATH = None  # menggunakan font bawaan Pillow agar deployment self-contained
MASTER_TEMPLATE = TEMPLATE_DIR / "rupiahtoto_master.png"
...
DEFAULT_THREAD_ID = int(os.getenv("MESSAGE_THREAD_ID", str(DEFAULT_CONFIG.get("message_thread_id", 8593))) or "0")
ADMIN_IDS = [
...
COOKIE_SECRET = os.getenv("SESSION_SECRET", DASHBOARD_PASSWORD + "-rupiahtoto-v1")
PORT = int(os.getenv("PORT", "8080"))
WIB = pytz.timezone("Asia/Jakarta")

logging.basicConfig(level=os.getenv("LOG_LEVEL", "INFO"), format="%(asctime)s | %(levelname)s | %(message)s")
log = logging.getLogger("rupiahtoto_result")
...
def template_for_market(pasaran: str) -> Optional[Path]:
    # Semua pasaran memakai satu master bersih. Nama pasaran ditulis dinamis.
    if MASTER_TEMPLATE.is_file():
...
def display_market_name(value: str) -> str:
    ...
    aliases = {
        "singapura": "SINGAPORE POOLS", "hongkong": "HONGKONG POOLS",
        "kingkong4d": "KINGKONG 4D POOLS", "kingkong4dp2": "KINGKONG 4D POOLS P2",
        "totomacau4dp1": "TOTOMACAU 4D POOLS P1", "totomacau4dp6": "TOTOMACAU 4D POOLS P6",
        "totomacau5dp1": "TOTOMACAU 5D POOLS P1", "totomacau5dp2": "TOTOMACAU 5D POOLS P2",
        "totomacaup2": "TOTOMACAU 4D POOLS P2", "totomacaup3": "TOTOMACAU 4D POOLS P3",
        "totomacaup4": "TOTOMACAU 4D POOLS P4", "totomacaup5": "TOTOMACAU 4D POOLS P5",
    }
    if key in aliases:
        return aliases[key]
    spaced = re.sub(r"(?<=\D)(?=\d)|(?<=\d)(?=\D)", " ", str(value)).replace("_", " ").replace("-", " ")
    return re.sub(r"\s+", " ", spaced).strip().upper() + " POOLS"


def _font(size: int):
    # Pillow 10.4 menyediakan font default scalable, jadi tidak perlu file .ttf eksternal.
    return ImageFont.load_default(size=max(8, int(size)))


def fit_font(text: str, max_width: int, start_size: int, min_size: int = 28):
    size = int(start_size)
...
def centered(draw, xy, text, font, fill, stroke_width=0, stroke_fill="black"):
    x1, y1, x2, y2 = xy
    box = draw.textbbox((0, 0), text, font=font, stroke_width=stroke_width)
    x = x1 + ((x2 - x1) - (box[2] - box[0])) / 2
    y = y1 + ((y2 - y1) - (box[3] - box[1])) / 2 - box[1]
...
def _rounded_panel(draw: ImageDraw.ImageDraw, xy, radius, fill, outline=None, width=1):
    """Panel penutup untuk area dinamis agar teks lama pada master tidak pernah bocor."""
    draw.rounded_rectangle(xy, radius=radius, fill=fill, outline=outline, width=width)


def _centered_shadow_text(draw, xy, text, font, fill, stroke_width=0, stroke_fill="#000000", shadow=3):
    x1, y1, x2, y2 = xy
    ...
    if shadow:
        draw.text((x + shadow, y + shadow), text, font=font, fill="#160D00", stroke_width=stroke_width, stroke_fill="#160D00")
    draw.text((x, y), text, font=font, fill=fill, stroke_width=stroke_width, stroke_fill=stroke_fill)
...
def make_result_image(pasaran: str, angka: str, row: Optional[ResultRow] = None, prefix="result") -> Path:
    ...
    dt = image_date(row)
    w, h = img.size
    box = lambda a, b, c, d: (int(w*a), int(h*b), int(w*c), int(h*d))

    market_text = display_market_name(pasaran)
    tgl_text = f"RESULT {dt.day:02d} {MONTHS_ID[dt.month - 1]} {dt.year}"

    # AREA DINAMIS 1 — PASARAN. Panel hitam/emas menutup teks contoh di master.
    market_panel = box(.188, .241, .812, .306)
    _rounded_panel(draw, market_panel, int(h*.018), "#090806", "#F9C846", max(2, int(w*.003)))
    inner_market = box(.202, .246, .798, .302)
    market_font = fit_font(market_text, int(w*.565), int(h*.052), int(h*.030))
    _centered_shadow_text(draw, inner_market, market_text, market_font, "#FFD85D", 2, "#6B3500", 2)

    # AREA DINAMIS 2 — TANGGAL. Dibuat bersih, kontras, dan selalu center.
    date_panel = box(.286, .313, .714, .357)
    _rounded_panel(draw, date_panel, int(h*.015), "#0C0B0A", "#EBC34B", max(2, int(w*.002)))
    date_font = fit_font(tgl_text, int(w*.390), int(h*.030), int(h*.020))
    centered(draw, date_panel, tgl_text, date_font, "#FFFFFF", 1, "#272727")

    # AREA DINAMIS 3 — ANGKA RESULT. Menutup angka contoh tanpa menimpa frame ornamen utama.
    result_panel = box(.206, .395, .794, .544)
    _rounded_panel(draw, result_panel, int(h*.030), "#E8BC42", "#FFE98C", max(3, int(w*.004)))
    # Lapisan kedua memberi efek depth tetapi tetap ringan untuk Railway.
    inner_result = box(.217, .405, .783, .534)
    _rounded_panel(draw, inner_result, int(h*.024), "#F5D76D", "#B77A16", max(2, int(w*.002)))
    result_font = fit_font(angka, int(w*.500), int(h*.132), int(h*.072))
    _centered_shadow_text(draw, inner_result, angka, result_font, "#111111", 2, "#F9E18A", 4)
...
async def send_result(pasaran: str, angka: str, row: Optional[ResultRow] = None, channel_id: Optional[str] = None):
    ...
    thread_id = int(cfg.get("message_thread_id") or DEFAULT_THREAD_ID or 0)
    ...
    market_label = display_market_name(pasaran)
    caption = f"""🔔 <b>HASIL ACUAN RESULT {html.escape(market_label)}</b> 🔔

<b>Pasaran :</b> {html.escape(market_label)}
<b>Result :</b> {angka}
<b>Shio :</b> {html.escape(shio)}
<b>Tanggal :</b> {dt.day:02d} {MONTHS_ID[dt.month - 1]} {dt.year}{period_line}

Terima Kasih &amp; Good Luck Bos ku
<b>SALAM PROFIT DARI RUPIAHTOTO</b> ❤️"""
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(
        InlineKeyboardButton("🔐 LOGIN RUPIAHTOTO", url=os.getenv("LOGIN_URL", "https://rupiahpondok.com/")),
        InlineKeyboardButton("💬 TELEGRAM RUPIAHTOTO", url=os.getenv("CS_URL", "https://t.me/rupiahtoto")),
        InlineKeyboardButton("💬 WHATSAPP RUPIAHTOTO", url=os.getenv("WA_URL", "https://api.whatsapp.com/send?phone=6281996142595")),
    )
    try:
        send_kwargs = {"caption": caption, "reply_markup": keyboard}
        if thread_id:
            send_kwargs["message_thread_id"] = thread_id
        sent = await bot.send_photo(target, InputFile(str(result_path)), **send_kwargs)
...
# ---------------- WEB ADMIN ----------------
def auth_token():
    return hmac.new(COOKIE_SECRET.encode(), b"rupiahtoto-admin", hashlib.sha256).hexdigest()


def is_web_auth(request: web.Request) -> bool:
    return hmac.compare_digest(request.cookies.get("rupiahtoto_auth", ""), auth_token())
...
async def login_get(request):
    return web.Response(text="""<!doctype html><html><head><meta name=viewport content='width=device-width,initial-scale=1'><title>Rupiah Toto Login</title><style>body{font-family:Arial;background:#111;color:#eee;display:grid;place-items:center;height:100vh;margin:0}.box{width:min(92vw,420px);background:#1b1b1b;padding:28px;border-radius:14px}input,button{box-sizing:border-box;width:100%;padding:13px;margin-top:12px;border-radius:8px;border:1px solid #444}button{background:#e6b800;font-weight:700;cursor:pointer}</style></head><body><form class=box method=post action=/login><h2>Rupiah Toto Admin</h2><input type=password name=password placeholder='Password dashboard' required><button>LOGIN</button></form></body></html>""", content_type="text/html")
...
async def login_post(request):
    ...
    resp.set_cookie("rupiahtoto_auth", auth_token(), httponly=True, samesite="Lax", max_age=60*60*24*30)
    return resp


async def logout(request):
    resp = web.HTTPFound("/login")
    resp.del_cookie("rupiahtoto_auth")
    return resp
...
def dashboard_html(cfg, state, notice=""):
    ...
    return f"""<!doctype html><html><head><meta name=viewport content='width=device-width,initial-scale=1'><title>Rupiah Toto Admin</title><style>
...
<h1>Rupiah Toto Auto Result</h1><p>Status AUTO: <b>{status}</b> | Last scan: {esc(state.get('last_scan') or '-')} | <a href=/logout>Logout</a></p>
...
<div><label>Topic / Thread ID</label><input type=number min=0 name=message_thread_id value='{esc(cfg.get('message_thread_id', 8593))}'></div>
...
async def _apply_form_to_config(request):
    ...
    cfg["message_thread_id"] = int(str(data.get("message_thread_id", cfg.get("message_thread_id", DEFAULT_THREAD_ID))).strip() or "0")
...
async def health(request):
    cfg, state = load_config(), load_state()
    return web.json_response({"ok": True, "bot": "rupiahtoto-result", "auto": bool(cfg.get("auto_enabled")), "last_scan": state.get("last_scan")})
