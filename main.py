...
def make_result_image(pasaran: str, angka: str, row: Optional[ResultRow] = None, prefix="result") -> Path:
    ...
    dt = image_date(row)
    # Koordinat dinormalisasi agar tetap tepat bila master diganti resolusinya.
    w, h = img.size
    ...
    # Hanya tiga bagian ini yang berubah. Semua teks promo lain sudah permanen di master.
    centered(draw, box(.12, .205, .88, .285), market_text, fit_font(market_text, int(w*.72), int(h*.061)), "#FFD95A", 4, "#4A2600")
    centered(draw, box(.29, .278, .71, .325), tgl_text, fit_font(tgl_text, int(w*.38), int(h*.027), 22), "white", 2, "#333333")
    centered(draw, box(.18, .345, .82, .51), angka, fit_font(angka, int(w*.54), int(h*.14), 70), "#FFF2A3", 5, "#1E1000")
